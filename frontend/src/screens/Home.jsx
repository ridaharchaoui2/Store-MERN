...
function Home() {
  ...
  const BasketBall = products?.filter(
    (item) => item.category?.toLowerCase() === "basketball",
  );
  const legendsCat = products?.filter(
    (item) => item.category?.toLowerCase() === "legends",
  );
        ...
        <section>
          <div className="flex flex-col items-center mb-12">
            ...
            <h1 className="text-center font-black text-4xl md:text-6xl tracking-tighter uppercase italic">
              Legends <span className="text-zinc-800 not-italic">Never</span>{" "}
              Die
            </h1>
